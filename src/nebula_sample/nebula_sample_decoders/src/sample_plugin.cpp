...
{
  sensor_id_ = config.frame_id.empty() ? "sample_sensor" : config.frame_id;

  // SampleDecoder only consumes network identity and field-of-view today. Other
  // SensorConfiguration fields are intentionally ignored by this reference adapter
  // until the sample decoder grows behavior that uses them.
...
SensorPacketResult SampleSensorDecoderRuntime::process_packet(const SensorPacketView & packet)
{
  ...
  // Progress counters are cumulative for this runtime instance: processed counts
  // every process_packet() call, matched counts packets accepted for the sample
  // UDP data channel, decoded counts successful unpack() calls, and output_count
  // counts completed sample point clouds.
