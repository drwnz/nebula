...
void SensorRegistry::load_registry(const std::vector<std::string> & search_paths)
{
  ...
  std::map<SensorModel, std::string> model_owners;
  for (const auto & [package_name, metadata] : registered_plugins_) {
    for (const auto & model : metadata.supported_models) {
      model_owners.emplace(model, package_name);
    }
  }
  ...
  for (const auto & file_path : collect_descriptor_files(search_paths)) {
    try {
      ...
      for (const auto & model : metadata->supported_models) {
        const auto [owner_it, model_inserted] = model_owners.emplace(model, metadata->package_name);
        if (!model_inserted && owner_it->second != metadata->package_name) {
          std::cerr << "Duplicate plugin support for sensor model '"
                    << sensor_model_to_string(model) << "' declared by package '"
                    << metadata->package_name << "' at " << metadata->descriptor_path
                    << "; existing package '" << owner_it->second
                    << "' may be selected first by find_plugin_for_model()" << std::endl;
        }
      }
...
std::shared_ptr<SensorPlugin> SensorRegistry::load_plugin(const SensorPluginMetadata & metadata)
{
  ...
  auto destroy_func =
    reinterpret_cast<DestroyFunc>(dlsym(library->handle, metadata.destroy_symbol.c_str()));
  if (!destroy_func) {
    std::cerr << "Failed to find destroy symbol '" << metadata.destroy_symbol << "' in "
              << metadata.library_path << ": " << dlerror() << std::endl;
    return nullptr;
  }
