...
public:
  ...
  //
  // Thread-safety: PacketRouter does not provide internal synchronization.
  // configure(), route(), and get_metrics() must be externally serialized, or
  // route() must be called from a single thread after configure() has returned.
