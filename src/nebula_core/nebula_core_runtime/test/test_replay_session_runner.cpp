...
void write_sample_descriptor(
  const fs::path & descriptor_path, const std::string & plugin_library_path)
{
...
TEST_F(TestReplaySessionRunner, ReplaySampleSensor)
{
  ...
  write_sample_descriptor(test_dir_ / "nebula_sample_decoders.json", plugin_library_path_);
  ...
  runner.set_output_callback([&](const SensorDecodedOutput & output) {
    ...
    EXPECT_EQ(output.sensor_id, "sample_frame");
  ...
  config.sensor_config.frame_id = "sample_frame";

  EXPECT_NO_THROW(runner.configure(config));
  runner.start();
  runner.wait_until_finished();
  EXPECT_TRUE(output_received);
  const auto metrics = runner.get_router_metrics();
  EXPECT_EQ(metrics.processed_packets, 10u);
  EXPECT_EQ(metrics.matched_packets, 10u);
  EXPECT_EQ(metrics.dropped_packets, 0u);
}

TEST_F(TestReplaySessionRunner, ThrowingOutputCallbackDoesNotEscapeReplay)
{
  if (plugin_library_path_.empty() || !fs::exists(plugin_library_path_)) {
    GTEST_SKIP() << "Sample plugin library not found";
  }
  ...
  ReplaySessionRunner runner(registry);
  runner.set_output_callback(
    [](const SensorDecodedOutput &) { throw std::runtime_error("test callback failure"); });
  ...
  EXPECT_NE(stderr_output.find("ReplaySessionRunner: output callback threw"), std::string::npos);
}
