...
TEST_F(TestSensorRegistry, LoadRegistryWarnsAboutDuplicateModelOwnership)
{
  std::ofstream first((test_dir_ / "a_plugin.json").string());
  first << R"({
    "vendor": "first_vendor",
    "package": "nebula_first_decoders",
    "library": "libfirst_plugin.so",
    "models": ["Pandar64"]
  })";
  first.close();
  ...
  second << R"({
    "vendor": "second_vendor",
    "package": "nebula_second_decoders",
    "library": "libsecond_plugin.so",
    "models": ["Pandar64"]
  })";
  second.close();
  ...
  EXPECT_NE(
    stderr_output.find("Duplicate plugin support for sensor model 'Pandar64'"), std::string::npos);
  EXPECT_EQ(registry.get_registered_plugins().size(), 2u);
}
...
TEST_F(TestSensorRegistry, LoadPluginRejectsAbiMismatch)
{
  SensorPluginMetadata metadata;
  metadata.package_name = "abi_mismatch";
  metadata.library_path = NEBULA_RUNTIME_TEST_ABI_MISMATCH_PLUGIN;
  ...
  EXPECT_NE(stderr_output.find("Plugin ABI version mismatch"), std::string::npos);
}

TEST_F(TestSensorRegistry, LoadPluginAcceptsMissingAbiVersionWithWarning)
{
  SensorPluginMetadata metadata;
  metadata.package_name = "missing_abi_version";
  metadata.library_path = NEBULA_RUNTIME_TEST_MISSING_ABI_VERSION_PLUGIN;
  metadata.factory_symbol = "create_nebula_sensor_plugin";
  metadata.destroy_symbol = "destroy_nebula_sensor_plugin";
  ...
  EXPECT_NE(stderr_output.find("does not export nebula_plugin_abi_version"), std::string::npos);
}
