...
| `nebula_sample_hw_interfaces` | Legacy sample hardware interface used by the ROS sample wrapper; not part of runtime plugin loading.      |
...
16 bytes for Linux `IFNAMSIZ`. Oversized addresses or interface names throw
`std::length_error` instead of truncating.
...
`SensorOutputSink` is an experimental alternative to `std::function` callbacks
for future RT-capable runtimes. The base `set_sink()` implementation is a
no-op, the sample decoder does not override it, and the current session hosts
(`LiveTransportGraph` and `ReplaySessionRunner`) do not wire it into their
output paths yet. Treat this as a reserved extension point rather than a
functional RT output path in the current runtime.

An RT-capable runtime can implement this interface and register it with
`set_sink()`:
...
The sink pointer must outlive the runtime it is registered with. The existing
`set_output_callback()` / `set_error_callback()` interface remains available
for non-RT use.
...
hosts (`LiveTransportGraph` and `ReplaySessionRunner`) and every packet source
(`UdpPacketSource`, `CanPacketSource`, `TcpPacketSource`, `HttpPacketSource`,
`PcapPacketSource`) catch the exception, log it to `stderr` with bounded
repetition, and continue running. Transport errors and decoder errors continue
to flow through the dedicated error callback with the appropriate
`SensorErrorType`; user-callback throws are not reported through the error
callback because they are programming errors in user code, not faults of the
runtime.
